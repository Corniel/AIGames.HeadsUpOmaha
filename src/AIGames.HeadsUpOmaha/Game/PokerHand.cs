...
namespace AIGames.HeadsUpOmaha.Game
{
	...
	public struct PokerHand : ISerializable, IXmlSerializable
	{
		...
		/// <summary>Gets the winning chance of a hand.</summary>
		/// <remarks>
		/// Based on a random test.
		/// </remarks>
		public static double GetWinningChanceHeadsUpOmaha(IEnumerable<Card> hand)
		{
			if (hand == null || hand.Count() != 4 || hand.Any(c => c.IsEmpty()))
			{
				throw new ArgumentException("The hand should contain 4 cards.", "hand");
			...
			for (int s = 0; s < simulations; s++)
			{
				...
				var oppo = deck.Skip(5).Take(2);

				var myBest = CreateFromHeadsUpOmaha(table, hand);
				var opBest = CreateFromHeadsUpOmaha(table, oppo);

				var compare = PokerHandComparer.Instance.Compare(myBest, opBest);
				if (compare == 1)
				{
					score += 2;
				}
			...
			return 0.5 * (double)score / (double)simulations;
		}
